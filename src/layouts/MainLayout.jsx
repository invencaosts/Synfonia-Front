  ...
  return (
    ...
    >
        ...
        <div className="p-6 border-t border-white/5 space-y-4">
          ...
          <div className="mt-4 pt-4 border-t border-(--border-subtle) space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-center gap-4">
                <a
                  href="https://www.instagram.com/invencaosts"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 text-dim hover:text-brand-legible transition-colors group"
                  title="Instagram"
                >
                  <Instagram size={14} className="group-hover:scale-110 transition-transform" />
                  <span className="text-[10px] font-bold tracking-tight">@invencaosts</span>
                </a>
                <a
                  href="https://www.linkedin.com/in/invencaosts"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-1.5 text-dim hover:text-[#0A66C2] transition-colors group"
                  title="LinkedIn"
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        {/* Conectar Spotify/YouTube — versão mobile (sidebar não existe aqui) */}
        {!isGuest && (!isSpotifyConnected || !isYoutubeConnected) && (
          <div className="md:hidden flex items-center gap-2 px-4 py-2 border-b border-(--border-subtle) overflow-x-auto shrink-0">
            {!isSpotifyConnected && (
              <button
                onClick={connectSpotify}
                className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#1DB954]/10 border border-[#1DB954]/20 text-[#1DB954] text-xs font-bold active:scale-95 transition-all"
              >
            ...
            {!isYoutubeConnected && (
              <button
                onClick={connectYoutube}
                disabled={youtubeAuthStatus === 'waiting'}
                className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-red-600/10 border border-red-600/20 text-red-500 text-xs font-bold active:scale-95 transition-all disabled:opacity-50"
              >
